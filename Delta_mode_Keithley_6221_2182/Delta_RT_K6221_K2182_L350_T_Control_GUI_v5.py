...
class Advanced_Delta_GUI:
    ...
    def setup_styles(self):
        ...
        mpl.rcParams.update({'font.family': 'Segoe UI', 'font.size': self.FONT_SIZE_BASE,
                             'axes.titlesize': self.FONT_SIZE_BASE + 4, 'axes.labelsize': self.FONT_SIZE_BASE + 2})
    ...
    def create_header(self):
        ...
        Label(header_frame, text="K6221/2182 & L350: Delta Mode R-T (T-Control)", bg=self.CLR_HEADER,
              fg=self.CLR_ACCENT_GOLD, font=font_title_italic).pack(side='left', padx=20, pady=10)
        ...
        Label(header_frame, text=f"Version: {self.PROGRAM_VERSION}", bg=self.CLR_HEADER,
              fg=self.CLR_FG_LIGHT, font=self.FONT_SUB_LABEL).pack(side='right', padx=20, pady=10)
    ...
    def create_info_frame(self, parent):
        ...
        ttk.Label(frame, text="UGC-DAE Consortium for Scientific Research", font=institute_font,
                  background=self.CLR_BG_DARK).grid(row=0, column=1, padx=10, pady=(10, 0), sticky='sw')
    ...
    def _load_logo(self, canvas):
        if PIL_AVAILABLE and os.path.exists(self.LOGO_FILE_PATH):
            try:
                img = Image.open(self.LOGO_FILE_PATH).resize((self.LOGO_SIZE, self.LOGO_SIZE),
    ...
    def create_input_frame(self, parent):
        ...
        self.entries["Sample Name"].grid(row=1, column=0, columnspan=2, padx=padx_val, pady=(0, 10), sticky='ew')
        ...
        self.entries["Start Temp"].grid(row=3, column=0, padx=(padx_val, 5), pady=(0, 5), sticky='ew')
        ...
        self.entries["End Temp"].grid(row=3, column=1, padx=(5, padx_val), pady=(0, 5), sticky='ew')
        ...
        self.entries["Rate"].grid(row=5, column=0, padx=(padx_val, 5), pady=(0, 10), sticky='ew')
        ...
        self.entries["Cutoff"].grid(row=5, column=1, padx=(5, padx_val), pady=(0, 10), sticky='ew')
        ...
        self.entries["Apply Current"].grid(row=7, column=0, padx=(padx_val, 5), pady=(0, 5), sticky='ew')
        ...
        self.entries["Compliance"].grid(row=7, column=1, padx=(5, padx_val), pady=(0, 5), sticky='ew')
        ...
        self.lakeshore_cb.grid(row=9, column=0, padx=(padx_val, 5), pady=(0, 10), sticky='ew')
        ...
        self.keithley_cb.grid(row=9, column=1, padx=(5, padx_val), pady=(0, 10), sticky='ew')
        ...
        ttk.Button(frame, text="Browse Save Location...", command=self._browse_file_location).grid(
            row=11, column=0, columnspan=2, padx=padx_val, pady=4, sticky='ew')

        self.start_button = ttk.Button(frame, text="Start Measurement", command=self.start_measurement,
                                       style='Start.TButton')
        self.start_button.grid(row=13, column=0, padx=padx_val, pady=(10, 10), sticky='ew')

        self.stop_button = ttk.Button(frame, text="Stop", command=self.stop_measurement,
                                      style='Stop.TButton', state='disabled')
        self.stop_button.grid(row=13, column=1, padx=padx_val, pady=(10, 10), sticky='ew')
    ...
    def create_graph_frame(self, parent):
        ...
        ttk.Checkbutton(top_bar, text="Log Resistance Axis", variable=self.log_scale_var,
                        command=self._update_y_scale).pack(side='right', padx=5)
        ...
        self.line_main, = self.ax_main.plot([], [], color=self.CLR_ACCENT_RED, marker='o', markersize=3,
        ...
        self.line_sub1, = self.ax_sub1.plot([], [], color=self.CLR_ACCENT_GOLD, marker='.', markersize=3,
                                            linestyle='-', animated=True)
        self.ax_sub1.set_xlabel("Temperature (K)")
        self.ax_sub1.set_ylabel("Voltage (V)")
        self.ax_sub1.grid(True, linestyle='--', alpha=0.6)

        self.line_sub2, = self.ax_sub2.plot([], [], color=self.CLR_ACCENT_GREEN, marker='.', markersize=3,
                                            linestyle='-', animated=True)
    ...
    def start_measurement(self):
        try:
            ...
            if not (self.params['start_temp'] < self.params['end_temp'] < self.params['cutoff']):
            ...
            writer.writerow(["Timestamp", "Elapsed Time (s)", "Temperature (K)", "Heater Output (%)",
                             "Measured Voltage (V)", "Resistance (Ohm)"])
    ...
    def _stabilization_loop(self):
        ...
        try:
            ...
            if abs(current_temp - self.params['start_temp']) < 0.1:
                ...
                self.is_stabilizing = False
                self.root.after(5000, self._start_hardware_ramp)  # Move to next stage
            else:
    ...
    def _update_measurement_loop(self):
        ...
        try:
            ...
            if temp >= self.params['cutoff']:
                ...
                self.root.after(900, self._update_measurement_loop)  # Slightly less than 1s to prevent drift
        except Exception as e:
