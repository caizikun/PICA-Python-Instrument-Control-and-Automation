...
class VT_GUI_Active:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(f"K2400/2182 & L350: R-T Sweep (T-Control) v{self.PROGRAM_VERSION}")
    ...
    def setup_styles(self):
        ...
        style.configure('TEntry', fieldbackground=self.CLR_INPUT_BG, foreground=self.CLR_FG_LIGHT,
        ...
        style.map('TButton', background=[('active', self.CLR_ACCENT_GOLD), ('hover', self.CLR_ACCENT_GOLD)],
                  foreground=[('active', self.CLR_BG_DARK), ('hover', self.CLR_BG_DARK)])
        ...
        style.configure('TLabelframe.Label', background=self.CLR_FRAME_BG, foreground=self.CLR_FG_LIGHT,
                        font=self.FONT_TITLE)
        style.configure('TCombobox', fieldbackground=self.CLR_INPUT_BG, foreground=self.CLR_FG_LIGHT,
                        arrowcolor=self.CLR_FG_LIGHT, selectbackground=self.CLR_ACCENT_BLUE,
    ...
    def create_widgets(self):
        ...
        ttk.Label(header, text="K2400/2182 & L350: R-T Sweep (T-Control)", style='Header.TLabel',
    ...
    def _create_info_panel(self, parent, grid_row):
        ...
        ttk.Label(frame, text="Mumbai Centre", font=institute_font,
                  background=self.CLR_FRAME_BG).grid(row=1, column=1, padx=10, pady=(0, 5), sticky='nw')
        ...
        ttk.Label(frame, text=details_text, justify='left',
                  background=self.CLR_FRAME_BG).grid(row=3, column=0, columnspan=2,
    ...
    def _create_right_panel(self, parent):
        ...
        self.line_main, = self.ax_main.plot([], [], color=self.CLR_ACCENT_RED,
    ...
    def _create_control_panel(self, parent, grid_row):
        ...
        self.stop_button = ttk.Button(button_frame, text="Stop", style='Stop.TButton',
                                      state='disabled', command=self.stop_experiment)
        self.stop_button.grid(row=0, column=1, sticky='ew', padx=5)
        ttk.Button(button_frame, text="Scan", command=self._scan_for_visa).grid(
    ...
    def _create_console_panel(self, parent, grid_row):
        ...
        self.console = scrolledtext.ScrolledText(frame, state='disabled', bg=self.CLR_CONSOLE_BG,
    ...
    def start_experiment(self):
        try:
            ...
            self.backend.connect(self.params['k2400_visa'], self.params['k2182_visa'], self.params['ls_visa'])
            self.backend.configure_instruments(self.params['current_ma'], self.params['compliance_v'])
            ...
            with open(self.data_filepath, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Temperature (K)", "Voltage (V)", "Elapsed Time (s)"])
        ...
        try:
            if self.experiment_state == 'stabilizing':
                ...
                with open(self.data_filepath, 'a', newline='') as f:
                    csv.writer(f).writerow([f"{temp:.4f}", f"{voltage:.6e}", f"{elapsed:.2f}"])
    ...
    def _validate_and_get_params(self):
        try:
            ...
            if not all([p for k, p in params.items() if k not in ['rate', 'cutoff']]):
                raise ValueError("A required field is empty.")
            if params['rate'] == 0:
                raise ValueError("Ramp Rate cannot be zero for an active sweep.")
